...
def prevent_sentence_segmentation(doc):
    ...
    return doc
# VAHAB TODO replace following two by
...
def parse_sentence_spacy(sentence_text, sentence_entities):
    ...
    for e in sentence_entities:
        ...
        # connecting tkens of an entity to eachother by "_" ex: "art galery" ---> "art_galery"
    ...
    #print(".... ----> parsed sentence by spacy = ", parsed)
...
def process_sentence_spacy(sentence, sentence_entities, sentence_pairs, positive_entities,
    ...
    """
    change mask_entities=True ====>  mask_entities=False to get rid of "drug" word instead of entitieas
    Process sentence to obtain labels, instances and classes for a ML classifier
    :param sentence: sentence processed by spacy ; is a spacy doc !!!
    ...
    graph, nodes_list = get_network_graph_spacy(sentence) # generate graph of SDP in networkx
    ...
    for (e1, e2) in combinations(sentence_head_tokens, 2):
        ...
        #print(sentence_head_tokens[e1].split("e")[-1])
        # reorder according to entity ID
        if int(sentence_head_tokens[e1].split("e")[-1]) > int(sentence_head_tokens[e2].split("e")[-1]):
            e1, e2 = e2, e1
        else:
            print(sentence_head_tokens[e1], e1, sentence_head_tokens[e2], e2)
            print(' = = = sentence_head_tokens[e1].split("e")[-1]= ',sentence_head_tokens[e1].split("e")[-1])
        # reorder according to entity orders TODO VAHAB
        if int(e1.split("-")[-1]) > int(e2.split("-")[-1]):
            e1, e2 = e2, e1
        else:
            print(sentence_head_tokens[e1], e1, sentence_head_tokens[e2], e2)
            print(' = = = e1.split("-")[-1])= ',e1.split("-")[-1])
        ...
        try:
            ...
            if len(sdp) > max_sdp_len or len(sdp) < min_sdp_len: #
            ...
            for i, element in enumerate(sdp):
                if element != "ROOT":
                    ...
                    #    negations += 1
                        ...
                        #vector.append("drug")
                    if mask_entities and sdp_token.idx in entity_offsets:
                        vector.append("ENTITY__"+sdp_token.text)# #vector.append("drug") each entity is masked and replaced by the "ENTITY" tag
                    else:
                        vector.append(sdp_token.text)
                    if wordnet_tags:    #TODO.. change to ontology tag (EL)
            ...
            if head_token_position is None:
                ...
                try:
                    ...
                    print(wordnet_vector)
                    print("Exception massage = ", e)
            ...
            left_wordnets.append([])
            right_wordnets.append([])
        except Exception as e: #defualt exception ...
            print("defaul Exception massage = ", e)
        ...
        if (sentence_head_tokens[e1], sentence_head_tokens[e2]) in sentence_pairs:
            ...
            print("......1 e1=", e1, "   e2=", e2)
            print("......2 sentence_pairs[(sentence_head_tokens[e1], sentence_head_tokens[e2])]=", sentence_pairs[(sentence_head_tokens[e1], sentence_head_tokens[e2])])

        else:
            classes.append(0)
        print("......3 classes=", classes)

        # doc = sentence # thes two rows added an removed by vahab; instead tokenized the sentence directly in "parse_semeval_sentences" method
        # sentence_tokenize = [sent.string.strip() for sent in doc.sents]
    # return labels, sentence_tokenize , (left_word_vectors, right_word_vectors), (left_wordnets, right_wordnets), classes, pos_gv, neg_gv
    if len(classes) == 0 : # TODO added by vahab to prevent to escape sentnces that dont have sdp and middle text;
        # TODO orijinal code removes sentences that dont have middle text such as following example in semeval2010
        # TODO since it has focused on SDP
        """
